/* Backchannel landing page — key modal + curl copy buttons
   CSP-safe: no inline event handlers, all listeners attached here. */
...
(function() {
  ...
  function issueKey() {
    ...
    var btn = document.getElementById('issue-key-btn');
    btn.disabled = true;
    ...
    .then(function(r) {
      ...
      if (r.ok) {
        ...
        result.innerHTML = '<div style="color:#5cff80;margin-bottom:8px;">&#10003; Key issued</div>'
          ...
          + '<button id="copy-key-btn" style="padding:4px 10px;border-radius:6px;border:1px solid #444;background:transparent;color:#8bcf90;font-family:var(--font-mono);font-size:0.72rem;cursor:pointer;white-space:nowrap;">copy</button>'
          + '</div>'
        ...
        document.getElementById('copy-key-btn').addEventListener('click', copyKeyValue);
      } else {
  ...
  // "Get a Test key" hero button
  document.getElementById('open-key-btn').addEventListener('click', openKeyModal);

  // Curl copy buttons
  var copyBtns = document.querySelectorAll('.curl-card-copy');
  for (var i = 0; i < copyBtns.length; i++) {
    copyBtns[i].addEventListener('click', function() { copyCurl(this); });
  }

  // Key modal: issue, close, enter-to-submit, backdrop click
  document.getElementById('issue-key-btn').addEventListener('click', issueKey);
  document.getElementById('close-key-btn').addEventListener('click', closeKeyModal);
  document.getElementById('agent-label-input').addEventListener('keydown', function(e) {
